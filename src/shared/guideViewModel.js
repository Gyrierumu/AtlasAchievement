(function attachGuideViewModel(root, factory) {
  ...
  function getExplicitGuideTrophyTags(trophy = {}, game = {}) {
    const slug = String(game?.slug || '').trim().toLowerCase();
    if (!['a-way-out', 'armored-core-vi-fires-of-rubicon', 'assassins-creed-mirage', 'assassins-creed-origins', 'assassins-creed-odyssey', 'assassins-creed-shadows', 'assassin-s-creed-valhalla', 'avatar-frontiers-of-pandora', 'baldurs-gate-3', 'beyond-two-souls', 'black-myth-wukong', 'celeste', 'clair-obscur-expedition-33', 'cyberpunk-2077', 'dark-souls-ii-scholar-of-the-first-sin', 'dark-souls-remastered', 'days-gone', 'demons-souls', 'detroit-become-human', 'disney-epic-mickey-rebrushed', 'dragons-dogma-2', 'final-fantasy-vii-remake', 'final-fantasy-vii-rebirth', 'final-fantasy-xvi', 'gran-turismo-7', 'hades', 'heavy-rain', 'hogwarts-legacy', 'hollow-knight-silksong', 'it-takes-two', 'lies-of-p', 'life-is-strange-double-exposure', 'life-is-strange-remastered', 'life-is-strange-true-colors', 'little-nightmares-ii', 'lords-of-the-fallen', 'metaphor-refantazio', 'monster-hunter-world', 'persona-3-reload', 'persona-5-royal', 'prince-of-persia-the-lost-crown', 'ratchet-and-clank-rift-apart', 'reanimal', 'resident-evil-6', 'resident-evil-7-biohazard', 'resident-evil-village', 'returnal', 'road-96', 'sekiro-shadows-die-twice', 'silent-hill-2-remake', 'split-fiction', 'star-wars-jedi-fallen-order', 'star-wars-jedi-survivor', 'stray', 'the-evil-within', 'the-witcher-3-wild-hunt', 'until-dawn'].includes(slug)) return [];
  ...
  function getGuideTrophyTags(trophy = {}, game = {}) {
    ...
    if (String(game?.slug || '').trim().toLowerCase() === 'life-is-strange-double-exposure') {
      const trophyId = String(trophy?.id || '').trim();
      const storyIds = new Set(['lis_de_split_decision', 'lis_de_bullet_proof', 'lis_de_face_to_facade', 'lis_de_fade_to_white', 'lis_de_tempest_fugit', 'lis_de_commencement']);
      const filteredTags = tags.filter(tag => tag?.id !== 'difficulty' && (tag?.id !== 'story' || storyIds.has(trophyId)));
      ids.delete('difficulty');
      if (!storyIds.has(trophyId)) ids.delete('story');
      tags.length = 0;
      tags.push(...filteredTags);
    }
  ...
  function getGuideNetworkRequirementModel(game = {}, viewModel = {}) {
    ...
    const explicitOnline = game?.onlineRequired === true || game?.requiresOnline === true || game?.hasMandatoryOnline === true;
    ...
    const hasOnline = explicitOnline ? true : (explicitNoOnline ? false : hasAffirmativeOnlineRequirement(inputs.online, onlineCount));
  ...
  function shouldReadRoadmapFirst(game = {}, trophies = [], roadmap = []) {
    ...
    const riskCounts = ['a-way-out', 'armored-core-vi-fires-of-rubicon', 'assassins-creed-mirage', 'assassins-creed-origins', 'assassins-creed-odyssey', 'assassins-creed-shadows', 'assassin-s-creed-valhalla', 'avatar-frontiers-of-pandora', 'beyond-two-souls', 'clair-obscur-expedition-33', 'cyberpunk-2077', 'dark-souls-ii-scholar-of-the-first-sin', 'demons-souls', 'detroit-become-human', 'final-fantasy-vii-remake', 'final-fantasy-vii-rebirth', 'final-fantasy-xvi', 'it-takes-two', 'lies-of-p', 'life-is-strange-double-exposure', 'life-is-strange-remastered', 'life-is-strange-true-colors', 'marvels-spider-man', 'marvels-spider-man-miles-morales', 'persona-3-reload', 'persona-5-royal', 'red-dead-redemption-2', 'returnal', 'road-96', 'sekiro-shadows-die-twice', 'silent-hill-2-remake', 'split-fiction', 'star-wars-jedi-fallen-order', 'stray', 'the-witcher-3-wild-hunt'].includes(String(game?.slug || '').trim().toLowerCase())
  ...
  function buildRouteChangingTrophies(trophies = [], game = {}) {
    ...
    if (['a-way-out', 'armored-core-vi-fires-of-rubicon', 'assassins-creed-mirage', 'assassins-creed-origins', 'assassins-creed-odyssey', 'assassins-creed-shadows', 'assassin-s-creed-valhalla', 'avatar-frontiers-of-pandora', 'beyond-two-souls', 'black-myth-wukong', 'cyberpunk-2077', 'demons-souls', 'final-fantasy-vii-remake', 'final-fantasy-vii-rebirth', 'final-fantasy-xvi', 'hades', 'life-is-strange-double-exposure', 'life-is-strange-remastered', 'life-is-strange-true-colors', 'little-nightmares-ii', 'metaphor-refantazio', 'monster-hunter-world', 'persona-3-reload', 'persona-5-royal', 'prince-of-persia-the-lost-crown', 'ratchet-and-clank-rift-apart', 'reanimal', 'resident-evil-6', 'returnal', 'road-96', 'sekiro-shadows-die-twice', 'split-fiction', 'star-wars-jedi-fallen-order', 'star-wars-jedi-survivor'].includes(attentionSlug) && Array.isArray(game?.attentionPoints)) {
  ...
  function buildCriticalTrophyAlerts(game = {}, trophies = []) {
    const normalizedSlug = String(game?.slug || '').trim().toLowerCase();
    if (['avatar-frontiers-of-pandora', 'beyond-two-souls', 'cyberpunk-2077'].includes(normalizedSlug) && Array.isArray(game?.attentionPoints) && game.attentionPoints.length) {
      return game.attentionPoints
        .map(item => ({
          name: item?.title || item?.name || '',
          label: Array.isArray(item?.tags) && item.tags.length ? String(item.tags[0]) : 'Atenção',
          reason: item?.detail || item?.tip || item?.text || item?.description || ''
        }))
  ...
  function buildGuideViewModel(game, completedSource = [], options = {}) {
    ...
    let riskCounts = ['a-way-out', 'armored-core-vi-fires-of-rubicon', 'assassins-creed-mirage', 'assassins-creed-origins', 'assassins-creed-odyssey', 'assassins-creed-shadows', 'assassin-s-creed-valhalla', 'avatar-frontiers-of-pandora', 'beyond-two-souls', 'clair-obscur-expedition-33', 'cyberpunk-2077', 'dark-souls-ii-scholar-of-the-first-sin', 'demons-souls', 'detroit-become-human', 'final-fantasy-vii-remake', 'final-fantasy-vii-rebirth', 'final-fantasy-xvi', 'it-takes-two', 'lies-of-p', 'life-is-strange-double-exposure', 'life-is-strange-remastered', 'life-is-strange-true-colors', 'little-nightmares-ii', 'marvels-spider-man', 'marvels-spider-man-miles-morales', 'metaphor-refantazio', 'monster-hunter-world', 'persona-3-reload', 'persona-5-royal', 'reanimal', 'red-dead-redemption-2', 'returnal', 'road-96', 'sekiro-shadows-die-twice', 'silent-hill-2-remake', 'split-fiction', 'star-wars-jedi-fallen-order', 'stray', 'the-witcher-3-wild-hunt'].includes(String(game?.slug || '').trim().toLowerCase())
    ...
    if (normalizedSlug === 'sekiro-shadows-die-twice') {
      ...
      Object.assign(sekiroEditorialById.sekiro_sekiro, {
        tip: 'Conclua os outros 33 trofeus da lista base. Gauntlets, Reflections, skins e conteudo de update ficam fora da platina.'
      });
      Object.assign(sekiroEditorialById.sekiro_master_of_the_prosthetic, {
        tip: 'Planeje materiais raros e Lapis Lazuli; Shura bloqueia Fountainhead Palace, entao deixe upgrades finais para rota de Kuro/NG+ e confira antes do cleanup.'
      });
      Object.assign(sekiroEditorialById.sekiro_height_of_technique, {
        tip: 'Farm de XP nao e perdivel. Guarde para areas finais ou NG+ e evite gastar skill points sem conferir as arvores necessarias.'
      });
      Object.assign(sekiroEditorialById.sekiro_peak_physical_strength, {
        tip: 'Exige todas as Prayer Beads: derrote mini-bosses sensiveis antes de avancos de historia e revise pendencias antes de NG+.'
      });
      Object.assign(sekiroEditorialById.sekiro_purification, {
        tip: 'Prepare Kuro, Emma e Hirata Estate revisitado antes de derrotar o chefe final; se faltar, deixe para backup ou NG+.'
      });
      Object.assign(sekiroEditorialById.sekiro_demon_of_hatred, {
        tip: 'Chefe opcional de endgame e parte de Man Without Equal; derrote antes de iniciar NG+ se ele ainda faltar na rota atual.'
      });
      Object.assign(sekiroEditorialById.sekiro_great_serpent, {
        tip: 'Evento opcional de exploracao com passos especificos. Use checklist no cleanup; se ficar pendente, resolva em nova passagem sem tratar como perda permanente.',
        is_missable: false,
        tags: [{ id: 'exploration', label: 'Exploracao', tone: 'partial' }, { id: 'event', label: 'Evento opcional', tone: 'partial' }, { id: 'cleanup', label: 'Cleanup', tone: 'neutral' }, { id: 'spoiler', label: 'Spoiler', tone: 'spoiler' }]
      });
      Object.assign(sekiroEditorialById.sekiro_great_colored_carp, {
        tip: 'Depende de Fountainhead Palace: garanta acesso pela rota de Kuro antes de Shura ou deixe o evento para NG+.'
      });
