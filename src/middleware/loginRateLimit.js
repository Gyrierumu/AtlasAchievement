...
function registerRateLimit(req, _res, next) {
  const key = `register:${buildKey(req)}`;
  const now = Date.now();
  const entry = attempts.get(key) || { timestamps: [], blockedUntil: 0 };
  ...
  if (entry.blockedUntil && entry.blockedUntil > now) {
    const waitSeconds = Math.ceil((entry.blockedUntil - now) / 1000);
    return next(new AppError(`Muitas tentativas de cadastro. Tente novamente em ${waitSeconds}s.`, 429));
  }
