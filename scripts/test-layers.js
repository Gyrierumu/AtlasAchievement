...
function validateData() {
  ...
  assert.deepStrictEqual(
    residentEvil2MythDestinations.map(destinations => destinations.map(destination => destination.href || '')),
    [
      ['#hardcore-s-rank-armas-infinitas', '#guideQuickPlan', '#guideChecklistPanel'],
      ['#hardcore-s-rank-armas-infinitas', '#runs-de-restricao', '#guideQuickPlan'],
      ['#the-4th-survivor-grim-reaper', '#guideDlcCompletionPanel', '#guideFaqPanel']
    ],
    'Mitos 4, 5 e 6 devem preservar destinos existentes e usar as quatro anchors estaveis da Fase 19'
  );
  ...
  assert.strictEqual(sampleGames.find(game => game.slug === 'resident-evil-5')?.commonMythsGuide?.myths?.length, 8, 'Resident Evil 5 deve manter os 8 mitos publicos, incluindo o custo zero dos trajes');
...
async function validateGuide(slug = '') {
  ...
  if (slug === 'resident-evil-5') {
    ...
    const instructionalVisuals = Array.isArray(seedGame.instructionalVisuals) ? seedGame.instructionalVisuals : [];
    ...
    assert.strictEqual(platinumCategories.length, 9, 'Extras da Platina de Resident Evil 5 deve manter 9 categorias, incluindo o compendio operacional');
    assert.deepStrictEqual(platinumCategories.map(category => category.id), [
      ...
      'bosses-critical-encounters'
    ], 'Extras da Platina de Resident Evil 5 deve manter as categorias anteriores e acrescentar o compendio operacional');
    ...
    assert(dlcText.includes('combo quebra se o contador entre kills zerar ou se você morrer') && dlcText.includes('matar jogadores não alimenta a corrente'), 'Keep the Good Times Rolling deve explicar manutencao e quebra do combo');
    assert(dlcText.includes('O combo de 40 é compartilhado pela dupla') && dlcText.includes('alternem kills'), 'It Takes Two to Tango deve trazer boost operacional em dupla');
    assert(dlcText.includes('Dano recebido reduz sua pontuação') && dlcText.includes('Public Assembly'), 'Survivors deve explicar origem/perda de pontos e mapa de boost');
    assert(dlcText.includes('A meta de 80.000 é a soma da equipe') && dlcText.includes('organize 4 jogadores'), 'Team Survivors deve explicar placar da equipe e organizacao 2v2');
    assert(dlcText.includes('total 134.000') && dlcText.includes('Wesker (S.T.A.R.S.) 50.000'), 'Personagens de Versus devem listar os oito pagos e custo total');
    assert(dlcText.includes('aparentemente disponível') && dlcText.includes('Não presuma matchmaking público ativo'), 'Status online deve permanecer inferencia e nao promessa de matchmaking');
    ...
    assert(dlcText.includes('mire 80.000 pontos agregados') && dlcText.includes('Rocket Launchers encontrados nos assist jumps'), 'The Great Escape deve documentar alvo e rota pratica do S rank');
    assert(dlcText.includes('conclua Desperate Escape no Veteran') && dlcText.includes('Final/landing pad') && dlcText.includes('Agitator Majini não é requisito de Run the Gauntlet'), 'Run the Gauntlet deve separar unlock, rota Professional e Agitators');
    const allDressedUp = viewModel.trophies.find(trophy => trophy.name === 'All Dressed Up');
    const bonusFeaturesCategory = platinumCategories.find(category => category.id === 'bonus-features-outfits-figures');
    const bonusFeaturesText = JSON.stringify(bonusFeaturesCategory);
    assert(allDressedUp?.tip.includes('Chris Outfit (Safari)') && allDressedUp.tip.includes('30 BSAA Emblems') && allDressedUp.tip.includes('0 Exchange Points'), 'All Dressed Up deve listar quatro trajes, emblemas e custo zero');
    assert(bonusFeaturesText.includes('re5-bonus-features-infinite-ammo') && bonusFeaturesText.includes('Special Settings') && bonusFeaturesText.includes('abaixo de 5 horas'), 'Bonus Features deve documentar Infinite Ammo e Infinite Rocket Launcher separadamente');
    ...
    const allTrophyNames = [...viewModel.trophies, ...dlcTrophies].map(item => item.name);
    ...
    assert.deepStrictEqual(editorialVideoLinks.map(link => link.label), [
      ...
      'Vídeo: BSAA Emblem #29 — Chapter 6-1 (8:40)',
    ...
    assert(!JSON.stringify(platinumCategories).includes('Vídeo: Lost in Nightmares') && !JSON.stringify(platinumCategories).includes('Vídeo: Desperate Escape'), 'Links de video de DLC nao devem entrar em Extras da Platina');
    assert.strictEqual(dlcTrophies.length, 20, 'Resident Evil 5 deve manter 20 trofeus DLC no checklist editorial');
    assert.strictEqual(new Set(viewModel.trophies.map(item => item.name)).size, 51, 'Resident Evil 5 deve manter 51 nomes base unicos');
    assert.strictEqual(new Set(dlcTrophies.map(item => item.name)).size, 20, 'Resident Evil 5 deve manter 20 nomes DLC unicos');
    assert.strictEqual(new Set(allTrophyNames).size, 71, 'Resident Evil 5 deve manter 71 nomes unicos entre base e DLC');
    assert.deepStrictEqual(seedGame.editorialDisplay, { faqMode: 'all', attentionMode: 'editorial' }, 'Resident Evil 5 deve declarar paridade editorial sem numeros fixos no renderer');
    assert.strictEqual(instructionalVisuals.length, 5, 'Resident Evil 5 deve manter cinco modulos visuais instrucionais prioritarios');
    assert.strictEqual(new Set(instructionalVisuals.map(visual => visual.id)).size, 5, 'Modulos visuais de Resident Evil 5 devem usar IDs unicos');
    assert.deepStrictEqual(instructionalVisuals.map(visual => visual.placement), [
      'dlc:re5-score-stars',
      'dlc:re5-agitators',
      'category:bsaa-emblems',
      'category:treasures',
    ...
    instructionalVisuals.forEach(visual => {
      ...
      assert(visual.caption.includes('Diagrama esquemático — não representa escala.'), `${visual.id} deve declarar natureza esquematica sem escala`);
      assert(/^\/assets\/guides\/resident-evil-5\/[a-z0-9-]+\.svg$/.test(visual.src), `${visual.id} deve usar SVG local dedicado ao guia`);
      assert(Number.isFinite(visual.width) && visual.width > 0 && Number.isFinite(visual.height) && visual.height > 0, `${visual.id} deve declarar dimensoes intrinsecas`);
      assert(Array.isArray(visual.textFallback) && visual.textFallback.length >= 3 && visual.textFallback.every(item => String(item || '').trim()), `${visual.id} deve oferecer alternativa textual operacional`);
      assert(/^#[A-Za-z][\w:.-]*$/.test(visual.relatedAnchor), `${visual.id} deve apontar para ancora interna valida`);
      const visualPath = path.join(ROOT, 'public', visual.src.replace(/^\/+/, ''));
      assert(fs.existsSync(visualPath), `${visual.id} deve apontar para asset existente`);
    ...
    assert.strictEqual(treasures.length, 50, 'Resident Evil 5 deve manter 50 tesouros');
    assert.strictEqual(scoreStars.length, 18, 'Lost in Nightmares deve manter 18 Score Stars');
    ...
    bsaaEmblems.forEach((item, index) => {
      ['area', 'landmark', 'angle', 'action', 'pointOfNoReturn', 'cleanup'].forEach(field => {
        assert(String(item[field] || '').trim(), `BSAA Emblem #${index + 1} deve documentar ${field}`);
      });
      assert.strictEqual(item.videoTimestampSeconds, [7, 27, 48, 63, 72, 91, 106, 126, 142, 165, 179, 204, 222, 230, 243, 255, 270, 289, 302, 324, 340, 365, 402, 419, 435, 448, 470, 487, 520, 542][index], `BSAA Emblem #${index + 1} deve manter timestamp auditado`);
      assert(item.links?.[0]?.url.includes(`&t=${item.videoTimestampSeconds}s`), `BSAA Emblem #${index + 1} deve linkar seu timestamp`);
    });
    scoreStars.forEach((item, index) => {
      ['area', 'landmark', 'angle', 'action', 'pointOfNoReturn', 'cleanup'].forEach(field => {
        assert(String(item[field] || '').trim(), `Score Star #${index + 1} deve documentar ${field}`);
      });
      assert.strictEqual(item.videoTimestampSeconds, [6, 17, 30, 45, 54, 70, 90, 104, 137, 150, 161, 174, 185, 196, 206, 217, 231, 246][index], `Score Star #${index + 1} deve manter timestamp auditado`);
      assert(item.links?.[0]?.url.includes(`&t=${item.videoTimestampSeconds}s`), `Score Star #${index + 1} deve linkar seu timestamp`);
    });
    agitators.forEach((item, index) => {
      ['area', 'landmark', 'trigger', 'action', 'risk', 'pointOfNoReturn', 'cleanup'].forEach(field => {
        assert(String(item[field] || '').trim(), `Agitator Majini #${index + 1} deve documentar ${field}`);
      });
      assert.strictEqual(item.videoTimestampSeconds, undefined, `Agitator Majini #${index + 1} nao deve inventar timestamp`);
    });
    ['Soul Gem', 'Heart of Africa', 'Diamond (Marquise)'].forEach(name => {
      const treasure = treasures.find(item => item.name === name);
      assert(treasure?.landmark && treasure?.action && treasure?.pointOfNoReturn && treasure?.cleanup, `${name} deve ter localizacao e recuperacao autossuficientes`);
    });
    assert.strictEqual(seedGame.videoAudit?.length, 5, 'Resident Evil 5 deve manter auditoria dos cinco rotulos de video');
    assert.deepStrictEqual(seedGame.videoAudit.map(item => item.youtubeId), ['qG94-12Nznk', 'qG94-12Nznk', 'XKfQyYb_hBY', '4KAJ6zfUNxc', 'Zxx5PkPYeuU'], 'Auditoria deve preservar os quatro videos e os cinco usos editoriais');
    ...
    assert(farmText.includes('Dinheiro da campanha compra armas e upgrades') && farmText.includes('Exchange Points compram figuras e Infinite Ammo') && farmText.includes('trajes de All Dressed Up custam 0 pontos'), 'Rotas de Farm deve separar dinheiro, Exchange Points e trajes gratuitos');
    ...
    assert.strictEqual(commonMyths.length, 8, 'Mitos e erros comuns de Resident Evil 5 deve manter 8 mitos publicos');
    ...
    [
      ...
      'trajes de All Dressed Up custam Exchange Points',
      '0 pontos',
      'Night Terrors',
      'Run the Gauntlet'
    ].forEach(text => {
  ...
  if (slug === 'resident-evil-2-remake') {
    ...
    const re2ChapterAnchors = Object.fromEntries((seedGame.chapterRouteGuide?.chapters || []).filter(chapter => chapter.anchorId).map(chapter => [chapter.chapter, chapter.anchorId]));
    assert.deepStrictEqual(re2ChapterAnchors, {
      'Hardcore, S rank e armas infinitas': 'hardcore-s-rank-armas-infinitas',
      'Runs de restrição': 'runs-de-restricao',
      'The 4th Survivor — Grim Reaper': 'the-4th-survivor-grim-reaper'
    }, 'Resident Evil 2 Remake deve manter somente as tres anchors estaveis da Fase 19');
    assert.strictEqual(new Set(Object.values(re2ChapterAnchors)).size, 3, 'Anchors das subsecoes da Fase 19 devem ser unicas');
    ...
    assert.strictEqual(re2Myths[3]?.where?.[0]?.href, '#hardcore-s-rank-armas-infinitas', 'Mito 4 deve apontar para Hardcore, S rank e armas infinitas');
    assert.strictEqual(re2Myths[4]?.where?.[0]?.href, '#hardcore-s-rank-armas-infinitas', 'Mito 5 deve apontar para Hardcore, S rank e armas infinitas');
    assert.strictEqual(re2Myths[4]?.where?.[1]?.href, '#runs-de-restricao', 'Mito 5 deve apontar para Runs de restricao');
    assert.strictEqual(re2Myths[5]?.where?.[0]?.href, '#the-4th-survivor-grim-reaper', 'Mito 6 deve apontar para The 4th Survivor');
    ...
    assert.strictEqual(re2Files.find(item => item.name === 'Confiscation Report')?.location, 'RPD 1F - Observation Room', 'Confiscation Report deve ficar exclusivamente na Observation Room');
    ...
    ['re2r_waist_space', 're2r_master_unlocking'].forEach(id => {
      ...
      const tags = guideModel.getGuideTrophyTags(trophy, seedGame).map(tag => tag.id);
      const displayTags = guideModel.getGuideTrophyDisplayTags(trophy, seedGame, 4).map(tag => tag.id);
      assert(!tags.includes('story') && tags.includes('missable') && tags.includes('collectible') && tags.includes('run'), `${id} deve preservar o token do filtro Perdivel e usar Coletavel e Perdivel por run, sem Historia`);
      assert(!displayTags.includes('missable') && displayTags.includes('collectible') && displayTags.includes('run'), `${id} nao deve renderizar tag Perdivel duplicada`);
    });
    const re2Customizer = seedGame.trophies.find(trophy => trophy.id === 're2r_customizer');
    assert.deepStrictEqual(guideModel.getGuideTrophyTags(re2Customizer, seedGame).map(tag => tag.id), ['missable', 'run', 'situational'], 'Customizer deve preservar o token do filtro Perdivel');
    assert.deepStrictEqual(guideModel.getGuideTrophyDisplayTags(re2Customizer, seedGame, 4).map(tag => tag.id), ['situational', 'run'], 'Customizer deve renderizar apenas Situacional e Perdivel por run');
    ['re2r_leon_s', 're2r_scarlet_hero'].forEach(id => {
      const tags = guideModel.getGuideTrophyTags(seedGame.trophies.find(trophy => trophy.id === id), seedGame).map(tag => tag.id);
      assert(tags.includes('rank-s') && tags.includes('difficulty') && tags.includes('run') && !tags.includes('story'), `${id} deve usar Rank, Dificuldade e Risco de run, sem Historia`);
    });
  ...
  await withTempApp(async ({ baseUrl, run, migrate }) => {
    ...
    if (slug === 'resident-evil-5') {
      ...
      const attentionPanelHtml = html.match(/<section id="guideEditorialNotesPanel"[\s\S]*?<section id="guideFaqPanel"/)?.[0] || '';
      const faqPanelHtml = html.match(/<section id="guideFaqPanel"[\s\S]*?<div id="guideCommentsSlot"/)?.[0] || '';
      ...
      assert.strictEqual(apiGame.faq?.length, 36, 'API de Resident Evil 5 deve expor 36 FAQs');
      assert.strictEqual(apiGame.attentionPoints?.length, 12, 'API de Resident Evil 5 deve expor 12 pontos de atencao');
      assert.strictEqual(apiGame.attention_count, 12, 'API de Resident Evil 5 deve alinhar attention_count com os 12 pontos editoriais');
      ...
      const instructionalVisualIds = apiGame.instructionalVisuals.map(visual => visual.id);
      assert.strictEqual((guideScopedHtml.match(/\sdata-instructional-visual="/g) || []).length, 5, 'SSR deve renderizar exatamente cinco modulos visuais');
      assert.strictEqual((extrasPanelHtml.match(/\sdata-instructional-visual="/g) || []).length, 3, 'Extras da Platina deve receber os três visuais da campanha base');
      assert.strictEqual((dlcPanelHtml.match(/\sdata-instructional-visual="/g) || []).length, 2, 'DLCs deve receber os visuais de Score Stars e Agitators');
      instructionalVisualIds.forEach(visualId => {
        assert.strictEqual((guideScopedHtml.match(new RegExp(`data-instructional-visual="${visualId}"`, 'g')) || []).length, 1, `${visualId} deve aparecer uma unica vez no SSR`);
      ...
      assert(farmPanelHtml.includes('Exchange Points compram figuras e Infinite Ammo') && farmPanelHtml.includes('trajes de All Dressed Up custam 0 pontos'), 'Rotas de Farm SSR deve separar pontos pagos e trajes gratuitos');
      ...
      ['Versus', '51 base + 20 DLC', 'Hand Grenade', 'Proximity Bomb', 'M93R', 'Hydra', 'S&amp;W M500', 'Score Stars', 'BSAA Emblems', 'All Dressed Up', '0 pontos', 'Night Terrors', 'Run the Gauntlet'].forEach(text => {
      ...
      assert(extrasPanelHtml.includes('9 categorias') || extrasPanelHtml.includes('9 categoria(s)'), 'Extras da Platina de Resident Evil 5 deve renderizar 9 categorias');
      ...
      assert(extrasPanelHtml.includes('Compêndio de chefes e encontros críticos') || extrasPanelHtml.includes('CompÃªndio de chefes e encontros crÃ­ticos'), 'Extras da Platina deve renderizar o compendio operacional');
      assert.strictEqual((extrasPanelHtml.match(/data-platinum-extra-category="bosses-critical-encounters"/g) || []).length, 22, 'Compendio SSR deve renderizar 22 encontros');
      ['Vídeo: BSAA Emblems — 30 localizações', 'Vídeo: BSAA Emblem #29 — Chapter 6-1 (8:40)', 'Vídeo: Heart of Africa — Chapter 5-3'].forEach(label => {
      ...
      assert(html.includes('Perguntas frequentes'), 'FAQ de Resident Evil 5 deve manter rotulo publico Perguntas frequentes');
      ...
      const renderedYoutubeIds = [...guideScopedHtml.matchAll(/https:\/\/www\.youtube\.com\/watch\?v=([^"&]+)/g)].map(match => match[1]);
      assert.deepStrictEqual([...new Set(renderedYoutubeIds)].sort(), ['4KAJ6zfUNxc', 'XKfQyYb_hBY', 'Zxx5PkPYeuU', 'qG94-12Nznk'].sort(), 'SSR deve usar somente os quatro videos auditados nos cinco usos editoriais');
    ...
    if (slug === 'resident-evil-2-remake') {
      ...
      assert.strictEqual((mythsPanelHtml.match(/href="#hardcore-s-rank-armas-infinitas"/g) || []).length, 2, 'Mitos 4 e 5 devem apontar para a anchor de Hardcore');
      assert.strictEqual((mythsPanelHtml.match(/href="#runs-de-restricao"/g) || []).length, 1, 'Mito 5 deve apontar para a anchor de runs de restricao');
      assert.strictEqual((mythsPanelHtml.match(/href="#the-4th-survivor-grim-reaper"/g) || []).length, 1, 'Mito 6 deve apontar para a anchor de The 4th Survivor');
      ...
      ['hardcore-s-rank-armas-infinitas', 'runs-de-restricao', 'the-4th-survivor-grim-reaper'].forEach(anchorId => {
        assert.strictEqual((routePanelHtml.match(new RegExp(`id="${anchorId}"`, 'g')) || []).length, 1, `Rota por capitulo deve renderizar uma unica anchor #${anchorId}`);
      });
      ...
      assert(mainHtml.includes('>5 alertas</span>') && !mainHtml.includes('5 alerta(s)'), 'Pontos de atencao deve usar pluralizacao natural para cinco alertas');
      const guideUiSource = readProjectFile('public/js/ui-guide.js');
      assert(guideUiSource.includes("routeTrophies.length === 1 ? 'alerta' : 'alertas'"), 'Renderizacao cliente deve tratar singular e plural de alerta dinamicamente');
      ['hardcore-s-rank-armas-infinitas', 'runs-de-restricao', 'the-4th-survivor-grim-reaper'].forEach(anchorId => {
        assert(guideUiSource.includes(`'${anchorId}': 'roadmap'`), `Navegacao cliente deve abrir a aba Roadmap para #${anchorId}`);
      });
      ...
      assert.strictEqual((html.match(/<strong>Requisito:<\/strong>/g) || []).length, 45, 'HTML publico deve rotular os 42 trofeus base e 3 extras como Requisito');
      assert(!html.includes('<strong>Descrição oficial:</strong>') && html.includes('42 troféus da base + 3 extras = 45 no total'), 'HTML publico nao deve chamar parafrase de descricao oficial e deve manter a terminologia de extras');
