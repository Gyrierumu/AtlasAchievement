...
- [x] Remediação incorporada em `release/resident-evil-5-v2-rc`
- [x] Node 20 validado
- [x] Instalação limpa validada
- [x] Contratos RE5 V2 verdes
- [x] Falha de RE2 provada como preexistente
- [x] Build verde
- [ ] Linux x64/Node 20 validado
- [ ] Staging aprovado
- [x] Segurança aprovada: produção com 0 crítica e 0 alta
...
npm run test:sqlite:native
npm run test:sqlite:runtime
...
No Bloco 7B não havia `STAGING_BASE_URL`, runner Linux, serviço dedicado ou credenciais de staging. O único serviço em `render.yaml` é produção; portanto, nenhum push, deploy, alteração de flag ou backup foi executado. Ver `resident-evil-5-v2-staging-validation.md`.
